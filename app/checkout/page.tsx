...
import { CheckCircle, MapPin, Phone, User, FileText } from 'lucide-react';
    ...
    if (success) {
        return (
            <div className="max-w-lg mx-auto px-4 py-16 text-center">
                ...
                <h1 className="text-2xl font-bold text-slate-800 mb-3">Đặt hàng thành công!</h1>
                <div className="bg-ocean-50 rounded-2xl p-5 mb-6">
                    <p className="text-slate-600 text-sm mb-2">Mã đơn hàng của bạn:</p>
                    <p className="text-2xl font-bold text-ocean-600">{success.orderId}</p>
                </div>
                <p className="text-slate-600 mb-6">
                    🎉 Nhân viên sẽ gọi xác nhận trong <strong>1 giờ</strong>. Cảm ơn bạn đã tin tưởng Hải Sản Quảng Ninh!
                </p>
                <div className="flex gap-3 justify-center">
                    <button
                        onClick={() => router.push(`/order/track?orderId=${success.orderId}`)}
                        className="px-6 py-3 bg-ocean-500 text-white font-semibold rounded-full hover:bg-ocean-600 transition-colors"
                    >
                        Tra cứu đơn hàng
                    </button>
                    <button
                        onClick={() => router.push('/')}
                        className="px-6 py-3 border border-ocean-300 text-ocean-600 font-semibold rounded-full hover:bg-ocean-50 transition-colors"
                    >
                        Về trang chủ
                    </button>
                </div>
